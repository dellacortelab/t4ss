...
def run(epsilon_r):
        ...
        conf = GromacsGroFile("../benz_octanol.gro")
        ...
        alchemical_particles = set(range(particle_size))
...
################################################################################
        ...
        u_kln = np.zeros([nstates,niterations], np.float64)

        kT = AVOGADRO_CONSTANT_NA * BOLTZMANN_CONSTANT_kB * integrator.getTemperature()
        
        for iteration in range(niterations):
                print('state %5d iteration %5d / %5d' % (place, iteration, niterations))
                ...
                for l in range(nstates):
                        ...
                        u_kln[l,iteration] = simulation.context.getState(getEnergy=True).getPotentialEnergy() / kT
                
                # Reset alchemical state
                simulation.context.setParameter('clambda', CLAMBDA)
                simulation.context.setParameter('vlambda', VLAMBDA)

        np.savetxt("u_kln.txt",u_kln)
        print('Note: the so-called u_kln is really only u_ln.')
